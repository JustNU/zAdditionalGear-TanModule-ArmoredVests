...
class CustomItems {
	static handleCustomItems(database, core, config, itemConfig, itemData)
	{
		...
		if (itemConfig["Armored Vests"]["AddGearTan_TacTec_Armor"]) {
			core.addItemRetexture("AddGearTan_TacTec_Armor", itemData["AddGearTan_TacTec_Armor"].BaseItemID, itemData["AddGearTan_TacTec_Armor"].BundlePath, false, config.AddToBots, itemData["AddGearTan_TacTec_Armor"].LootWeigthMult);
			const dbItem = database.templates.items["AddGearTan_TacTec_Armor"];
			...
			// find handbook entry
			const dbItemHandbook = database.templates.handbook.Items.find((item) => {return item.Id === "AddGearTan_TacTec_Armor"});
			...
			// change flea price (if it has one)
			if (database.templates.prices["AddGearTan_TacTec_Armor"])
				database.templates.prices["AddGearTan_TacTec_Armor"] = dbItemHandbook.Price;
			
			// add trade offer
			if (config.EnableTradeOffers)
				core.createTraderOffer("AddGearTan_TacTec_Armor", "5ac3b934156ae10c4430e83c", "5449016a4bdc2d6f028b456f", dbItemHandbook.Price, 4);
		}
	}
}
